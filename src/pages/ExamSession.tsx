...
const EXAM_CONFIGS: Record<string, { name: string; questionCount: number; durationMinutes: number; examType: string }> = {
  "enem-rapido": { name: "ENEM Rapido", questionCount: 30, durationMinutes: 90, examType: "ENEM" },
  "enem-completo": { name: "ENEM Completo", questionCount: 90, durationMinutes: 300, examType: "ENEM" },
  "fuvest": { name: "Fuvest 1a Fase", questionCount: 45, durationMinutes: 150, examType: "Fuvest" },
...
const ExamSession = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const perSubject: PerSubjectScore[] = Object.entries(subjectScores).map(([subject, s]) => ({
      subject,
      correct: s.correct,
      total: s.total,
    ...
    await supabase.from("exam_results").insert({
      ...
      time_spent_seconds: (config.durationMinutes * 60) - timeLeft,
      per_subject_scores: perSubject,
    });
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (selected) {
        await supabase.from("answer_history").insert({
          ...
          is_correct: q.options.find(o => o.label === selected)?.is_correct || false,
    ...
    if (prof) {
      await supabase.from("profiles").update({
        ...
        exams_completed: (prof.exams_completed || 0) + 1,
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center">
          <p className="text-muted-foreground">Banco de questoes insuficiente para este simulado.</p>
          <Link to="/exams" className="mt-4 inline-flex text-sm font-semibold text-primary">Voltar</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background px-4 py-8">
        <div className="max-w-lg mx-auto">
          <div className="text-center animate-fade-in">
            ...
            <h1 className="text-2xl font-bold text-foreground">{config.name} — Resultado</h1>
            <div className={`mt-6 inline-flex items-center justify-center h-28 w-28 rounded-full ${getBg(results.score)} ${getColor(results.score)} text-4xl font-bold`}>
              {results.score}%
            </div>
            <p className="mt-3 text-sm text-muted-foreground">
              {results.correct} de {results.total} corretas • Tempo: {formatTime((config.durationMinutes * 60) - timeLeft)}
            </p>
          </div>

          <div className="mt-8 space-y-3 animate-fade-in" style={{ animationDelay: "0.1s" }}>
            <h2 className="text-base font-semibold text-foreground">Desempenho por Materia</h2>
            {results.perSubject.map(s => (
              <div key={s.subject} className="p-4 bg-card rounded-xl shadow-rest">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-foreground">{s.subject}</span>
                  <span className={`text-sm font-bold px-2 py-0.5 rounded-full ${getBg(s.percent)} ${getColor(s.percent)}`}>
                    {s.percent}%
                  </span>
                </div>
                <div className="h-2 bg-muted rounded-full">
                  <div className={`h-2 rounded-full transition-all duration-700 ${getBar(s.percent)}`} style={{ width: `${s.percent}%` }} />
                </div>
          ...
          <div className="mt-8 flex gap-3 justify-center animate-fade-in" style={{ animationDelay: "0.2s" }}>
            <Link to="/exams" className="px-6 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">
              Mais Simulados
            </Link>
            <Link to="/desempenho" className="px-6 py-2.5 rounded-lg bg-card shadow-rest text-foreground text-sm font-medium">
              Ver Performance
            </Link>
          </div>
        </div>
      ...
      <header className="sticky top-0 z-40 bg-card shadow-rest">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="flex items-center justify-between h-14">
            ...
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-primary/5 text-primary">{config.name}</span>
            <span className={`flex items-center gap-1 text-sm font-mono font-semibold ${timerColor}`}>
              <Clock className="h-4 w-4" />{formatTime(timeLeft)}
            </span>
          </div>
          <div className="h-1 bg-muted rounded-full -mt-1 mb-1">
            <div className="h-1 bg-primary rounded-full transition-all" style={{ width: `${(answeredCount / questions.length) * 100}%` }} />
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-6 max-w-3xl">
                ...
                <button
                  ...
                  className={`w-full p-4 rounded-xl text-left transition-all duration-200 flex items-start gap-3 ${
                    isSelected
                      ...
                      : "bg-card shadow-rest hover:shadow-interactive"
                  }`}
                >
                  <span className={`h-7 w-7 shrink-0 rounded-lg flex items-center justify-center text-xs font-bold ${
        ...
        <div className="mt-8 flex items-center justify-between">
          <button
            ...
            className="h-10 px-4 rounded-lg bg-card shadow-rest text-sm font-medium text-foreground disabled:opacity-30 flex items-center gap-1"
          >
          ...
          <button
            onClick={toggleFlag}
            className={`h-10 px-3 rounded-lg text-sm font-medium flex items-center gap-1 ${
              flagged.has(currentIndex) ? "bg-warning/10 text-warning" : "bg-card shadow-rest text-muted-foreground"
            }`}
          >
          ...
          {currentIndex < questions.length - 1 ? (
            ...
            <button
              ...
              className="h-10 px-5 rounded-lg bg-success text-success-foreground text-sm font-semibold"
            >
        ...
        <div className="mt-6">
          ...
          >
            {showNav ? "Esconder" : "Ver"} mapa de questoes
          </button>
          {showNav && (
              ...
              {questions.map((_, i) => (
                <button
                  ...
                  className={`h-8 w-8 rounded-lg text-xs font-semibold transition-all ${
                    ...
                    answers[i] ? "bg-success/10 text-success" :
                    flagged.has(i) ? "bg-warning/10 text-warning" :
                    "bg-card shadow-rest text-muted-foreground"
                  }`}
                >
                  {i + 1}
                </button>
