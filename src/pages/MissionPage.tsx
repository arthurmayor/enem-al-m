...
const MissionPage = () => {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center max-w-sm animate-fade-in">
          ...
          <h1 className="text-2xl font-extrabold text-foreground">Missão Concluída!</h1>
          {score.total > 0 && (<div className={`mt-4 inline-flex items-center justify-center h-20 w-20 rounded-full ${scoreColor} text-2xl font-extrabold`}>{finalPercent}%</div>)}
          ...
          <div className="mt-8 flex gap-3 justify-center">
            <Link to="/dashboard" className="px-6 py-2.5 rounded-xl gradient-bg text-primary-foreground text-sm font-semibold hover:opacity-90 transition-all">Voltar ao Dashboard</Link>
            <Link to="/study" className="px-6 py-2.5 rounded-xl bg-card border border-border/50 text-foreground text-sm font-medium hover:shadow-interactive transition-all">Mais Missões</Link>
          </div>
        </div>
      </div>
    );
  ...
  if (questions.length === 0) {
    return (<div className="min-h-screen bg-background flex items-center justify-center px-4"><div className="text-center max-w-sm"><p className="text-muted-foreground">Sem questões disponíveis para este tópico.</p><p className="text-xs text-muted-foreground mt-2">{mission?.subject} — {mission?.subtopic}</p><Link to="/dashboard" className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-primary"><ArrowLeft className="h-4 w-4" /> Voltar</Link></div></div>);
  }
      ...
      <header className="sticky top-0 z-40 bg-card/90 backdrop-blur-xl border-b border-border/50">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="flex items-center justify-between h-14">
            ...
            <span className="text-sm font-bold text-foreground">{currentIndex + 1} de {questions.length}</span>
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-primary/10 text-primary">{mission?.subject}</span>
          </div>
          <div className="h-1.5 bg-muted rounded-full -mt-1 mb-1"><div className="h-1.5 gradient-bg rounded-full transition-all duration-500" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }} /></div>
        </div>
      </header>

      <main className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="animate-fade-in" key={currentIndex}>
          <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">{mission?.subtopic}</p>
          ...
          {selectedOption && currentQuestion.explanation && (
            <div className="mt-6 p-4 bg-primary/5 rounded-2xl border border-primary/10 animate-fade-in">
