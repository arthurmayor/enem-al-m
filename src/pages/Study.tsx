      ...
      <main className="container mx-auto px-4 py-6 max-w-3xl">
        {missions.length === 0 ? (
          <div className="text-center py-16 animate-fade-in">
            ...
            >
              Fazer diagnóstico <ChevronRight className="h-4 w-4" />
            </Link>
          </div>
        ) : (
          <>
              ...
              {(["all", "pending", "completed"] as const).map((f) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${
                    filter === f ? "bg-primary text-primary-foreground" : "bg-card shadow-rest text-foreground"
                  }`}
                >
            ...
            <div className="space-y-3">
              {filtered.map((mission, i) => {
                ...
                const dateLabel = new Date(mission.date + "T12:00:00").toLocaleDateString("pt-BR", {
                  weekday: "short",
                  day: "numeric",
                  month: "short",
                });
                ...
                return (
                  <Link
                    ...
                    className="group flex items-center justify-between p-5 bg-card rounded-xl shadow-rest hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-300 animate-fade-in"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <h3 className={`mt-0.5 font-semibold text-sm ${isCompleted ? "text-muted-foreground line-through" : "text-foreground"}`}>
                          {mission.subtopic}
                        </h3>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12">
                <p className="text-sm text-muted-foreground">
                  {filter === "completed" ? "Nenhuma missão concluída ainda." : "Todas as missões foram concluídas!"}
                </p>
              </div>
            )}
          </>
