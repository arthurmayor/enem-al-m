...
const subjectColors: Record<string, string> = { "Matemática": "bg-primary", "Português": "bg-secondary", "Física": "bg-warning", "Química": "bg-success", "Biologia": "bg-success", "História": "bg-destructive", "Geografia": "bg-warning" };
      ...
      <header className="sticky top-0 z-40 bg-card/90 backdrop-blur-xl border-b border-border/50">
        <div className="container mx-auto flex h-14 items-center justify-between px-4 max-w-3xl">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-foreground">Estudar</span>
          </div>
      ...
      <main className="container mx-auto px-4 py-6 max-w-3xl">
        {missions.length === 0 ? (
          <div className="text-center py-16 animate-fade-in">
            ...
            <h2 className="text-lg font-bold text-foreground">Nenhuma missão agendada</h2>
            ...
            <Link to="/diagnostic/intro" className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-primary">Fazer diagnóstico <ChevronRight className="h-4 w-4" /></Link>
          </div>
        ) : (
          <>
            <div className="flex gap-2 mb-6 animate-fade-in">
              {(["all", "pending", "completed"] as const).map((f) => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${filter === f ? "gradient-bg text-primary-foreground" : "bg-card border border-border/50 text-foreground hover:border-primary/30"}`}>
              ...
              {filtered.map((mission, i) => {
                ...
                return (
                  <Link key={mission.id} to={`/mission/${mission.mission_type}/${mission.id}`}
                    className="group flex items-center justify-between p-5 bg-card rounded-2xl border border-border/50 hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-300 animate-fade-in"
                    style={{ animationDelay: `${i * 0.03}s` }}>
                    <div className="flex items-start gap-3">
                      {isCompleted ? <CheckCircle2 className="h-5 w-5 text-success shrink-0 mt-0.5" /> : <Circle className="h-5 w-5 text-muted-foreground shrink-0 mt-0.5" />}
                      <div>
                        <div className="flex items-center gap-2">
                          <div className={`h-2 w-2 rounded-full ${subjectColors[mission.subject] || "bg-primary"}`} />
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{mission.subject}</span>
                        </div>
                        <h3 className={`mt-0.5 font-semibold text-sm ${isCompleted ? "text-muted-foreground line-through" : "text-foreground"}`}>{mission.subtopic}</h3>
                        <div className="mt-2 flex items-center gap-2">
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-semibold">{missionTypeLabels[mission.mission_type] || mission.mission_type}</span>
